/**
 * SAMRAT GLOBAL — CLIENT INTERACTIVE LOGIC
...
function sendEstimateViaWhatsApp() {
  ...
  const serviceTitle = state.service === 'events' ? 'SAMRAT Event Production (Bangalore & India)' : (state.service === 'marketing' ? 'SAMRAT Digital Growth' : 'SAMRAT Corporate Advisory');
  const tierTitle = state.tier === 'standard' ? 'Growth / Imperial' : 'VIP Sovereign';

  const message = `Hello Ismail Kazia | SAMRAT Team! 👑%0A%0AI customized an instant quote on your portal and would like to lock this custom package:%0A%0A📌 *Division:* ${encodeURIComponent(serviceTitle)}%0A🎯 *Tier:* ${encodeURIComponent(tierTitle)}%0A📊 *Scale/Scope:* ${encodeURIComponent(state.scaleValue)} ${encodeURIComponent(conf.unit)}%0A💰 *Estimated Quote:* ${encodeURIComponent(currentPrice)}%0A%0APlease share the next steps and consultation schedule!`;
